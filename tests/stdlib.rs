...
fn string_predicates_and_pad() {
    assert_eq!(run(r#"<?php echo str_contains("hello world", "o w") ? "y" : "n";"#), "y");
    assert_eq!(run(r#"<?php echo str_starts_with("phplang", "php") ? "y" : "n";"#), "y");
    assert_eq!(run(r#"<?php echo str_ends_with("script.php", ".php") ? "y" : "n";"#), "y");
...
fn string_case_and_words() {
    assert_eq!(run(r#"<?php echo ucwords("the quick brown fox");"#), "The Quick Brown Fox");
...
fn string_encoding_helpers() {
    assert_eq!(run(r#"<?php echo htmlspecialchars("<a href=\"x\">&");"#), "&lt;a href=&quot;x&quot;&gt;&amp;");
    assert_eq!(run(r#"<?php echo chr(65), ord("A");"#), "A65");
    assert_eq!(run(r#"<?php echo dechex(255), " ", hexdec("ff");"#), "ff 255");
    assert_eq!(run(r#"<?php echo bin2hex("AB");"#), "4142");
}

#[test]
fn str_split_builds_array() {
    assert_eq!(run(r#"<?php echo implode("-", str_split("abcd", 2));"#), "ab-cd");
    assert_eq!(run(r#"<?php echo count(str_split("hello"));"#), "5");
}

#[test]
fn number_format_groups_and_rounds() {
    assert_eq!(run(r#"<?php echo number_format(1234567.891, 2);"#), "1,234,567.89");
    assert_eq!(run(r#"<?php echo number_format(1000);"#), "1,000");
    assert_eq!(run(r#"<?php echo number_format(1234.5678, 2, ".", " ");"#), "1 234.57");
}
...
fn array_slice_and_reverse() {
    assert_eq!(run(r#"<?php echo implode(",", array_slice([10, 20, 30, 40, 50], 1, 3));"#), "20,30,40");
    assert_eq!(run(r#"<?php echo implode(",", array_reverse([1, 2, 3]));"#), "3,2,1");
    assert_eq!(run(r#"<?php echo implode(",", array_slice([1, 2, 3, 4], -2));"#), "3,4");
}

#[test]
fn array_merge_renumbers_and_overwrites() {
    assert_eq!(run(r#"<?php echo implode(",", array_merge([1, 2], [3, 4]));"#), "1,2,3,4");
...
fn array_key_and_search() {
    assert_eq!(run(r#"<?php echo array_key_exists("x", ["x" => 1]) ? "y" : "n";"#), "y");
    assert_eq!(run(r#"<?php echo array_key_exists("z", ["x" => 1]) ? "y" : "n";"#), "n");
    assert_eq!(run(r#"<?php echo array_search(30, [10, 20, 30]);"#), "2");
}

#[test]
fn array_unique_and_flip() {
    assert_eq!(run(r#"<?php echo implode(",", array_values(array_unique([1, 2, 2, 3, 3, 3])));"#), "1,2,3");
...
fn array_fill_and_combine() {
    assert_eq!(run(r#"<?php echo implode(",", array_fill(0, 3, "x"));"#), "x,x,x");
...
fn array_diff_and_intersect() {
    assert_eq!(run(r#"<?php echo implode(",", array_values(array_diff([1, 2, 3, 4], [2, 4])));"#), "1,3");
    assert_eq!(run(r#"<?php echo implode(",", array_values(array_intersect([1, 2, 3, 4], [2, 4, 5])));"#), "2,4");
}

#[test]
fn json_encode_scalars_lists_and_maps() {
    assert_eq!(run(r#"<?php echo json_encode([1, 2, 3]);"#), "[1,2,3]");
    assert_eq!(run(r#"<?php echo json_encode(["a" => 1, "b" => 2]);"#), r#"{"a":1,"b":2}"#);
    assert_eq!(run(r#"<?php echo json_encode("he\"llo");"#), r#""he\"llo""#);
    assert_eq!(run(r#"<?php echo json_encode(true), json_encode(null);"#), "truenull");
}
...
fn var_export_and_boolval() {
    ...
    assert_eq!(run(r#"<?php echo boolval(0) ? "y" : "n", boolval("x") ? "y" : "n";"#), "ny");
}
