...
pub const CORPUS: &[Entry] = &[
    ...
    (
        ...
        "Language construct",
        "unset($var[, $var …]): void",
        "Removes variables, array elements, and object properties. Like `isset`, it is a construct rather than a call, so an index path such as `$a[$k]` is unset in place rather than being evaluated to a value first.",
    ...
    (
        ...
        "Called when a value is used as a function. DIVERGENCE: the runtime only honours it on a `Closure` handle (`src/host.rs`). Declaring `__invoke` on a user class does NOT make its instances callable — `$obj(…)` fails with \"value is not callable\".",
        "$c = function () { return 8; }; echo $c->__invoke();   // => 8",
    ),
    (
        "__toString",
        "Magic method",
    ...
    (
        ...
        "Rounding mode: halves to the nearest even value. Defined but not honoured by `round()`.",
        "",
    ),
    (
        "PHP_ROUND_HALF_ODD",
        ...
        "Rounding mode: halves to the nearest odd value. Defined but not honoured by `round()`.",
        "",
    ),
    (
        "E_ERROR",
    ...
    (
        ...
        "Predefined constant",
        "E_STRICT: int = 2048",
        "Error level for interoperability suggestions.",
        "",
    ),
    (
        "E_DEPRECATED",
    ...
    (
        ...
        "Predefined constant",
        "E_ALL: int = 32767",
        "Every error level. `error_reporting()` returns exactly this value and ignores its argument.",
    ...
    (
        ...
        "Bit combined with `SORT_STRING` or `SORT_NATURAL` to fold case. Defined but not honoured.",
        "",
    ),
    (
        "COUNT_NORMAL",
    ...
    (
        ...
        "Flag asking `preg_match`/`preg_match_all` to report byte offsets alongside each capture. DIVERGENCE: not implemented for those two functions — only `preg_split` honours its own offset flag.",
        "",
    ),
    (
        ...
        "Flag asking for non-participating groups to report null. DIVERGENCE: not implemented; unmatched interior groups report an empty string.",
        "",
    ),
    (
        "PREG_SPLIT_NO_EMPTY",
    ...
    (
        ...
        "Predefined constant",
        "JSON_HEX_TAG: int = 1",
        "`json_encode()` flag escaping `<` and `>`. DIVERGENCE: the core `json_encode` arm reads no flags, so this has no effect.",
        "",
    ),
    (
        "JSON_HEX_AMP",
        "Predefined constant",
        "JSON_HEX_AMP: int = 2",
        "`json_encode()` flag escaping `&`. Defined but not honoured.",
        "",
    ),
    (
        "JSON_HEX_APOS",
    ...
    (
        ...
        "Predefined constant",
        "JSON_HEX_QUOT: int = 8",
        "`json_encode()` flag escaping `\"` as `\\u0022`. Defined but not honoured.",
        "",
    ),
    (
        "JSON_FORCE_OBJECT",
    ...
    (
        ...
        "`json_encode()` flag encoding numeric strings as numbers. Defined but not honoured.",
        "",
    ),
    (
        ...
        "`json_encode()` flag leaving `/` unescaped. Defined but not honoured.",
        "",
    ),
    (
        "JSON_PRETTY_PRINT",
        ...
        "`json_encode()` flag requesting indented output. DIVERGENCE: not honoured — the encoder always emits the compact form.",
        "echo json_encode([\"a\" => 1], JSON_PRETTY_PRINT);   // => {\"a\":1}",
    ),
    (
        "JSON_UNESCAPED_UNICODE",
        "Predefined constant",
        "JSON_UNESCAPED_UNICODE: int = 256",
        "`json_encode()` flag emitting literal UTF-8 instead of `\\uXXXX`. Defined but not honoured.",
        "",
    ),
    (
        "JSON_THROW_ON_ERROR",
    ...
    (
        ...
        "`json_decode()` flag decoding objects as arrays. Redundant here: `json_decode` always produces arrays because the `$associative` argument is ignored.",
        "",
    ),
    (
    ...
    (
        ...
        "Concatenates the string with itself `$times` times. A negative count is clamped to zero, yielding the empty string.",
    ...
    (
        ...
        "Escapes `&`, `<`, `>`, `\"`, and `'` for HTML. DIVERGENCE: the `$flags`, `$encoding`, and `$double_encode` arguments are not read, so quote handling cannot be changed and `&` is always re-encoded.",
    ...
    (
        ...
        "Shares the `htmlspecialchars` arm. DIVERGENCE: it escapes only those same five characters, not PHP's full named-entity table.",
        "",
    ),
    (
        "strcmp",
    ...
    (
        ...
        "Pairs one array's values as keys with another's as values. Mismatched lengths are an error.",
    ...
    (
        ...
        "Rounds to `$precision` decimal places, halves away from zero. DIVERGENCE: the PHP 8.4 `$mode` argument is not read, so the `PHP_ROUND_HALF_*` constants have no effect.",
    ...
    (
        ...
        "Counts non-overlapping matches inside the byte window `[$offset, $offset + $length)`. Negative offsets and lengths count from the end. An empty needle raises `substr_count(): Argument #2 ($needle) cannot be empty`. The window is a raw byte slice, so a multibyte haystack can be cut mid-character.",
    ...
    (
        ...
        "Appends the separator after every `$length` units. DIVERGENCE: chunking counts CODEPOINTS, not bytes as PHP does, so multibyte input chunks differently. A non-positive length raises a `ValueError`-style message.",
    ...
    (
        ...
        "Decodes exactly `&lt;`, `&gt;`, `&quot;`, `&#039;`, `&#39;`, `&apos;`, and finally `&amp;`. DIVERGENCE: the `$flags` argument is not read, so quote entities are always decoded.",
    ...
    (
        ...
        "Shares the `htmlspecialchars_decode` arm. DIVERGENCE: only those seven entities are decoded, not PHP's full named and numeric entity table, and both `$flags` and `$encoding` are ignored.",
        "",
    ),
    (
        "strip_tags",
        ...
        "Drops every character between `<` and `>` using a nesting depth counter. DIVERGENCE: the `$allowed_tags` allow-list is not read, so nothing is preserved, and the depth model treats stray `<` inside attribute values differently from PHP's tag scanner.",
    ...
    (
        ...
        "Splits into chunks of `$length` CODEPOINTS; the final chunk may be shorter. A length below 1 raises `mb_str_split(): Argument #2 ($length) must be greater than 0`.",
    ...
    (
        ...
        "Non-overlapping match count in codepoints. An empty needle raises `mb_substr_count(): Argument #2 ($needle) cannot be empty`.",
    ...
    (
        ...
        "Pads to `$length` CODEPOINTS by cycling the pad string. All three `STR_PAD_*` modes are honoured, with `STR_PAD_BOTH` flooring the left side. An empty pad string raises only when padding is actually needed.",
    ...
    (
        ...
        "Splits into a list of sub-arrays of at most `$size` elements. A size below 1 raises `array_chunk(): Argument #2 ($length) must be greater than 0`.",
    ...
    (
        ...
        "Builds an array from the named variables, pulling each bound value out of the current scope. A name may itself be an array of names, recursively. Unset names are skipped.",
    ...
    (
        ...
        "Decimal division; a zero divisor raises `bcdiv(): Division by zero`. DIVERGENCE: the quotient is computed at a 100-significant-digit context before truncation, so this is BOUNDED precision — a `$scale` needing more than about 100 significant digits is not reliable.",
    ...
    (
        ...
        "The remainder `a - b * trunc(a / b)`, following PHP 7.2+ semantics. A zero divisor raises `bcmod(): Modulo by zero`. The intermediate division inherits the same bounded context as `bcdiv`.",
        "echo bcmod(\"10\", \"3\");   // => 1",
    ),
    (
        "bcpow",
        ...
        "Exponentiation by squaring on exact decimals. DIVERGENCE: a fractional exponent is silently truncated toward zero instead of raising PHP 8's `ValueError`, and an exponent beyond the 64-bit range saturates. A negative exponent yields the reciprocal, and zero to a negative power raises `bcpow(): Division by zero`.",
    ...
    (
        ...
        "The square root truncated to `$scale` fractional digits. A negative operand raises `bcsqrt(): Argument must be greater than or equal to 0`. DIVERGENCE: computed at the same bounded 100-significant-digit context as `bcdiv`.",
    ...
    (
        ...
        "Modular exponentiation at TRUE arbitrary precision — all three operands are truncated to integers and a big-integer `modpow` does the work. A zero modulus or a negative exponent raises. Because the result is an integer, `$scale` only controls how many trailing zero digits are appended.",
    ...
    (
        ...
        "Integer quotient truncated toward zero; a zero divisor raises `gmp_div_q(): Division by zero`. DIVERGENCE: `$rounding_mode` is accepted and ignored — only truncation is implemented.",
    ...
    (
        ...
        "A remainder whose sign follows the DIVISOR: the raw remainder has `|m|` added when it is negative, so a positive modulus always yields a result in `[0, |m|)`. A zero modulus raises.",
        "echo gmp_mod(\"-7\", \"3\");   // => 2",
    ),
    (
        "gmp_pow",
    ...
    (
        ...
        "Modular exponentiation at true arbitrary precision. A zero modulus raises `Modulus may not be zero` and a negative exponent raises `Negative exponent not supported`.",
    ...
    (
        ...
        "Digests the data with one of exactly six algorithms: `md5`, `sha1`, `sha256`, `sha512`, `crc32b`, `crc32`. DIVERGENCE: `sha384` is NOT accepted here even though `hash_hmac`, `hash_file`, and `hash_pbkdf2` accept it. An unknown name raises `hash(): Argument #1 ($algo) must be a valid hashing algorithm`. PHP 8.1's `$options` array does not exist.",
    ...
    (
        ...
        "RFC 2104 HMAC over the same digests, with the block size paired per algorithm. Exactly five names are accepted — `md5`, `sha1`, `sha256`, `sha384`, `sha512` — so the CRC variants `hash()` allows are rejected. An unknown name raises `hash_hmac(): Argument #1 ($algo) must be a valid cryptographic hashing algorithm`.",
    ...
    (
        ...
        "PBKDF2 per RFC 2898 over the local HMAC, accepting `md5`, `sha1`, `sha256`, `sha384`, `sha512`. `$length` is the length of the OUTPUT STRING — derived bytes for binary output, hex characters otherwise — and 0 emits the full digest. Raises on an unknown algorithm, non-positive iterations, a negative length, or a length above 16777216.",
    ...
    (
        ...
        "DIVERGENCE — NOT A CSPRNG. No OS-entropy dependency is available to this crate, so the bytes come from a SplitMix64 generator seeded from wall-clock nanoseconds XOR the process id. Use it for tests and filler only, never for keys, tokens, or salts. Output is the Latin-1 raw mapping, so `strlen()` exceeds `$length` whenever a generated byte reaches 0x80. A length below 1, or above 16 MiB, raises.",
        "",
    ),
    ...
    (
        ...
        "DIVERGENCE: the argument is never read and no level is stored, so it reports the same value before and after any attempted set. Always returns 32767 (`E_ALL`).",
        "error_reporting(E_ERROR); echo error_reporting();   // => 32767",
    ),
    (
        "ini_get",
        "System and runtime environment",
        "ini_get(string $option): false",
        "DIVERGENCE: always false — PHP's \"no such option\" result — for every name. The argument is never read.",
        "var_dump(ini_get(\"precision\"));   // => bool(false)",
    ),
    (
        "ini_set",
        "System and runtime environment",
        "ini_set(string $option, mixed $value): false",
        "DIVERGENCE: sets nothing and always returns false, PHP's failure value. Neither argument is read.",
        "var_dump(ini_set(\"precision\", \"3\"));   // => bool(false)",
    ),
    (
        "spl_object_id",
        "System and runtime environment",
