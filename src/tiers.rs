...
//!
//! `php --tiers script.php` prints the report.
...
pub struct ChunkTiers {
    /// Which chunk this is — `main` for a whole PHP program.
    pub name: String,
...
/// has gone round enough times to be recorded. phplang writes program output
...
/// host, so both compiles emit byte-identical ops. That is what lets fusevm's
...
///
/// A PHP program is not one chunk: the top level, every `function`, every
/// method, each class constant / property initializer, each `enum` case's
/// backing value, and every arm of a `try` lower to their own chunk. A hot loop
/// usually lives in one of the latter, so reporting only `main` would answer
/// the wrong question.
fn program_chunks(prog: &crate::compiler::Program) -> Vec<(String, Chunk)> {
    ...
    for (cname, c) in &prog.classes {
        for (mname, m) in &c.methods {
            out.push((format!("method {cname}::{mname}"), m.chunk.clone()));
        }
        for (n, ch) in &c.consts {
            out.push((format!("const {cname}::{n}"), ch.clone()));
        }
        for (n, ch) in c.prop_defaults.iter().chain(&c.static_prop_defaults) {
            out.push((format!("prop {cname}::${n}"), ch.clone()));
        }
        for (n, ch) in &c.enum_cases {
            if let Some(ch) = ch {
                out.push((format!("case {cname}::{n}"), ch.clone()));
            }
        }
    }
    for (i, t) in prog.try_defs.iter().enumerate() {
        out.push((format!("try #{i}"), t.try_chunk.clone()));
        for (j, c) in t.catches.iter().enumerate() {
            out.push((format!("try #{i} catch[{j}]"), c.chunk.clone()));
        }
        if let Some(ch) = &t.finally_chunk {
